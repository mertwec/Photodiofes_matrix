...
def main(size_display: int = 200):

    SIZE_DISPLAY = size_display
    data_pd = generate_random_data(2)
    ...
    data_pd_one = [[1, 1], [1, 1]]
    data_pd_1 = [[0.1, 0.1], [0.3, 0.7]]
    data_pd_2 = [[0.5, 0.5], [0.2, 0.2]]
    ...
    ax.axvline(x=0, color="green", linestyle="--", linewidth=1)
    ...
    point_label = ax.text(
        0, 0, "", color="white", fontsize=8, ha="left", va="bottom", visible=False
    )
    ...
    def update(point: Point2D | None):
        if point is None:
            ...
            point_label.set_position(
                (point.x + 1, point.y + 1)
            )  # смещение, чтобы не перекрывать точку
