...
class SensorConfig:
    ...
    # редакция 1.1. Задаются спецификацией — менять только вслед за ней.
    # Кадр обмена (§3): AA 55 | LEN | блок данных LEN байт | CRC_hi CRC_lo.
    UART_SOF = b"\xaa\x55"  # синхробайты 0xAA, 0x55
    UART_LEN_DATA = 30  # длина блока данных, редакция 1.1 (в 1.0 было 22)
